...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] sError, int err_max)
{
	...
	CreateNative("Hardpoint_GetDatabase", Native_GetDatabase);

	RegPluginLibrary("hardpoint");
...
public void OnPluginStart()
{
	...
	fwd_OnDatabaseConnected = CreateGlobalForward("OnDatabaseConnected", ET_Ignore, Param_Cell);
...
{
	if(g_tMoveHardpoint != INVALID_HANDLE) KillTimer(g_tMoveHardpoint);
	delete fwd_OnDatabaseConnected; fwd_OnDatabaseConnected = INVALID_HANDLE;
}
...
int GetRandomHardpoint()
{
	int[] hardpoints = new int[g_Hardpoints + 1];
	int hardpointCount = 0;

	for(int i = 0; i < g_Hardpoints; i++)
	{
		if(i == g_CurrentCycle) continue;
		hardpoints[i] = i;
		hardpointCount++;
	}

	SortIntegers(hardpoints, g_Hardpoints - 1, Sort_Random);
	return hardpoints[GetRandomInt(0, g_Hardpoints - 1)];
}
...
{
	//In Order:
	if(g_Hardpoint_Cycle.IntValue == 1)
	{
	...
	//Random Select:
	else if(g_Hardpoint_Cycle.IntValue == 2)
	{
		g_CurrentCycle = GetRandomHardpoint();
	}
...
public void sql_ConnectCB(Handle driver, Handle db, const char[] error, any data)
{
	...
	Call_OnDatabaseConnected(g_Database);
	return;
}
...
public void Call_OnDatabaseConnected(Handle hDatabase)
{
	Call_StartForward(fwd_OnDatabaseConnected);
...
{
	if (g_Database == INVALID_HANDLE) return view_as<int>(INVALID_HANDLE);
	else return view_as<int>(CloneHandle(g_Database, plugin));
}
